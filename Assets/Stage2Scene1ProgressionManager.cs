...
namespace Pattern.Quest.Alpha.Phases.Games
{
    public class Stage2Scene1ProgressionManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!runOnce)
            {
                if (slot1.correctPlacement && slot2.correctPlacement && slot3.correctPlacement && slot4.correctPlacement && slot5.correctPlacement && slot6.correctPlacement)
                {
            ...
            if (!runTwice)
            {
                if (slot1.inCorrectPlacement || slot2.inCorrectPlacement || slot3.inCorrectPlacement || slot4.inCorrectPlacement || slot5.inCorrectPlacement || slot6.inCorrectPlacement)
                {
                    textMan.positionChanged = true; // Directly set positionChanged
                    textMan.arrayPos = 11;
                    //    exitTrigger.gameObject.SetActive(true);
